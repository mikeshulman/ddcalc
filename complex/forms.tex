...
\begin{defn}
  ...
  \item A \textbf{differential 2-form} is an expression of the form $A\,\dx\wdg\dy$, where $A$ is a function of $x$ and $y$.
  \end{itemize}
\end{defn}
...
\begin{defn}
  ...
  \item A \textbf{differential 2-form} is an expression of the form $A\,\dx\wdg\dy + B \,\dy\wdg\dz + C \,\dz\wdg\dx$, where $A$, $B$, and $C$ are functions of $x$, $y$, and $z$.
  \item A \textbf{differential 3-form} is an expression of the form $A \,\dx\wdg\dy\wdg\dz$, where $A$ is a function of $x$, $y$, and $z$.
  \end{itemize}
\end{defn}
...
\begin{equation}
  \int_\region f(x,y) \,\dx\wdg \dy.\label{eq:2form-2D-integral}
\end{equation}
This is the integral of a differential 2-form (namely $f(x,y) \,\dx\wdg \dy$) over a 2-manifold (namely $\region$).
...
So what is the deal with this funny symbol $\wdg$?
...
You might think we should write the result simply as $\dx\,\dy$, but we stick a $\wdg$ symbol in between to remind us that this ``multiplication'' is not exactly like ordinary multiplication.
The main difference is that the generating symbols $\dx$ and $\dy$ do not commute; they \emph{anticommute}:
\begin{equation}
  \dy\wdg\dx = - \dx\wdg \dy.\label{eq:wedge-anticommute}
\end{equation}
A good way to think about this is with reference to orientations.
The 2-form $\dx\wdg\dy$ produces positive results when integrated over ``canonically'' (i.e. counterclockwisely) oriented regions, in which the direction of rotation is \emph{from} the positive $x$-axis \emph{towards} the positive $y$-axis.
Similarly, $\dy\wdg\dx$ produces positive results when integrated over oriented regions in which the direction of rotation is \emph{from} the positive $y$-axis \emph{towards} the positive $x$-axis --- but this is the opposite orientation, so we must have \cref{eq:wedge-anticommute}.

We also have
\[ \dx \wdg \dx = 0 \qquad\text{and}\qquad \dy\wdg \dy = 0. \]
This is analogous to \cref{eq:wedge-anticommute}; the direct analogy would be $\dx\wdg\dx = - \dx\wdg\dx$, but of course this implies $\dx\wdg\dx=0$.
...
Now we'd like this kind of ``differentiation'' to satisfy a product rule, with respect to the $\wdg$ product of differential forms; thus the differential of $f(x)\,\dx$ should be
\[ \dd(f(x)\,\dx) =  \df \wdg\dx + f(x)\, \dd(\dx). \]
Actually, since we are using the $\wdg$ product in the product rule, it's better to include it as a reminder in the notation for this differential as well.
Thus we will write instead
\[ \extd(f(x)\,\dx) =  \df \wdg\dx + f(x)\, \extd\dx. \]
...
It turns out that we should define $\extd\dx=0$; a mnemonic for this is that just as $\dx\wdg \dx = 0$, so also ``$\extd\dd = 0$''.

Now we can compute the differential of a general 1-form $f(x,y)\,\dx + g(x,y)\,\dy$ in two dimensions:
\begin{align*}
  \extd(f\,\dx + g\,\dy)
  &= \df \wdg \dx + \dg \wdg\dy\\
  &= (f_x\, \dx + f_y\, \dy) \wdg \dx + (g_x \,\dx+g_y\,\dy)\wdg\dy \\
  &= f_x\, \dx\wdg\dx + f_y\, \dy\wdg \dx + g_x \,\dx \wdg\dy + g_y\,\dy\wdg\dy \\
  &= (g_x - f_y)\, \dx\wdg\dy.
\end{align*}
...
\[ \int_{\region} (g_x - f_y)\, \dx\wdg\dy = \int_{\bdry\region} f\,\dx + g\,\dy. \]
...
Now we have more general differential 2-forms such as $A\,\dx\wdg\dy + B\,\dy\wdg\dz + C \,\dz\wdg\dx$.
Any two 1-forms have a $\wdg$ product, which satisfies identities such as $\dy\wdg\dz = -\dz\wdg\dy$ and $\dy\wdg\dy=0$, and so on.
Every 1-form has a differential, computed as
\begin{align*}
  \extd(f\,\dx + g\,\dy + h\,\dz)
  &= \df\wdg\dx + \dg\wdg\dy + \dh\wdg\dz\\
  &= (f_x \,\dx + f_y \,\dy + f_z\,\dz)\wdg\dx + (g_x \,\dx + g_y \,\dy + g_z\,\dz)\wdg\dy\\
  &\qquad + (h_x \,\dx + h_y \,\dy + h_z\,\dz)\wdg\dz\\
  &= f_y \,\dy\wdg\dx + f_z\,\dz\wdg\dx + g_x \,\dx\wdg\dy + g_z\,\dz \wdg\dy \\
  &\qquad + h_x\,\dx\wdg\dz + h_y \,\dy\wdg\dz\\
  &= (g_x - f_y)\,\dx\wdg\dy + (h_y - g_z) \,\dy\wdg\dz + (f_z - h_x)\,\dz\wdg\dx.
\end{align*}
...
We also have 3-forms such as $f(x,y,z)\, \dx\wdg\dy\wdg\dz$.
Every 2-form has a differential, computed as
\begin{align*}
  \extd(A\,\dx\wdg\dy + & B\,\dy\wdg\dz + C \,\dz\wdg\dx)\\
  &= \dd A \wdg \dx \wdg \dy + \dd B \wdg \dy \wdg \dz + \dd C \wdg \dz \wdg \dx\\
  &= A_z\, \dz \wdg \dx \wdg \dy + B_x \,\dx \wdg \dy \wdg \dz + C_y \,\dy\wdg \dz \wdg \dx\\
  &= (A_z + B_x + C_y)\, \dx\wdg\dy\wdg \dz
\end{align*}
(since $\dz \wdg \dx \wdg \dy = -\dx \wdg \dz \wdg \dy = \dx \wdg \dy \wdg \dz$ and similarly).
...
\begin{equation}
  \int_{\region} (A_z + B_x + C_y)\, \dx\wdg\dy\wdg \dz = \int_{\bdry\region} A\,\dx\wdg\dy + B\,\dy\wdg\dz + C \,\dz\wdg\dx.\label{eq:divergence-theorem}
\end{equation}
...
However, in three dimensions, a vector field can also be interpreted as a \emph{2-form}, namely $f\,\dy\wdg\dz + g \,\dz\wdg\dx + h \,\dx\wdg\dy$: each component is associated to the $\wdg$ of all the $\dd?$s for the \emph{other} variables.
The differential of this 2-form is the 3-form $(\dvg{\vc{F}})\,\dx\wdg\dy\wdg\dz$, where $\dvg{\vc{F}}$ is the \emph{divergence} from vector calculus.
...
\begin{enumerate}
\item If we have two vector fields $\vc{F}$ and $\vc{G}$, we can interpret them as 1-forms, take their $\wdg$ product, then \emph{reinterpret} the resulting 2-form as a vector field.
